...
@bp.route('/email/send', methods=['POST', 'OPTIONS'])
def send_email():
    """
    Send an email notification using Flask-Mail
    Expects JSON with: to_email, listing_title, listing_price, listing_category, buyer_netid
    """
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        data = request.get_json()
        current_app.logger.info(f"Email request received: {data}")
        
        # Validate required fields
        required_fields = ['to_email', 'listing_title', 'listing_price', 'buyer_netid']
        ...
        buyer_netid = data['buyer_netid']
        
        # Create email message
        subject = f"Interest in your listing: {listing_title}"
        body = f"""
        ...
        Buyer: {buyer_netid}@princeton.edu
        ...
        # Send the email
        mail.send(msg)
        current_app.logger.info(f"Email sent to {recipient_email} regarding listing {listing_title}")
...
def request_to_buy(listing_id):
    """
    Record a buyer's request to purchase a listing and update listing status to pending
    """
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        data = request.get_json()
        current_app.logger.info(f"Buy request received for listing {listing_id}: {data}")
        
        # Validate required fields
        if 'buyerId' not in data:
            return jsonify({'error': 'Buyer ID is required'}), 400
            
        buyer_netid = data['buyerId']
        ...
        try:
            ...
            if listing.status != 'available':
                return jsonify({'error': f'Listing is not available (current status: {listing.status})'}), 400
            
            # Get the buyer user
            buyer = User.query.filter_by(netid=buyer_netid).first()
            if not buyer:
                current_app.logger.warning(f"Buyer not found for netid: {buyer_netid}")
                # We'll still continue since the email might have been sent
            ...
            if buyer:
                try:
                    ...
                    current_app.logger.warning(f"Could not set buyer_id: {str(e)}")
                    # Continue anyway, at least the status will be updated
            
            # Save changes
            db.session.commit()
            
            current_app.logger.info(f"Listing {listing_id} status updated to pending for buyer {buyer_netid}")
            return jsonify({
                'success': True, 
        ...
        # Return a success response anyway since the email was likely sent
        return jsonify({
            'success': True, 
            'message': 'Your request has been received but could not be fully processed in the database',
            'listing_id': listing_id,
            'error_details': str(e)
        }), 200
